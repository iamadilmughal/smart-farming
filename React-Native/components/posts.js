...
export default class Posts extends Component {
    ...
    constructor() {
      ...
      getData = async()=>{
        ...
        const id = await AsyncStorage.getItem('post')
          ...
          const response = await fetch('http://192.168.43.19:3000/community/post/'+id);
 ...
 upvotes=async()=>{
   ...
   fetch("http://192.168.43.19:3000/community/upvote/",{
      method:"PUT",
      headers: {
 ...
 down = async()=>{
  ...
  fetch("http://192.168.43.19:3000/community/downvote/",{
    method:"PUT",
    headers: {
     'Content-Type': 'application/json'
   },
 ...
 alert(data)
  fetch("http://192.168.43.19:3000/community/comment",{
      method:"POST",
      headers: {
       'Content-Type': 'application/json'
     }, body:JSON.stringify({
 ...
 isImage(){
  ...
  const a = this.colors[themeColorIndex]
    ...
    <Text>{this.state.countup}</Text>
          ...
          <Button rounded style={{marginTop:15,backgroundColor:'#006400'}} onPress={()=>this.uploadComment()}>
            <Text>Post</Text>
          </Button>
          </Card>
        </Content>
